...
screen invert:
    add Invert(delay):
        size (1280, 720)
    timer delay:
        action PauseAudio("music")
    timer delay:
        action Play("sound", "sfx/glitch1.ogg")
    timer length + delay:
        action Hide("invert")
    on "show":
        action Function(hide_windows_enabled, enabled=False)
    on "hide":
        action PauseAudio("music", False)
    on "hide":
        action Stop("sound")
    on "hide":
        action Function(hide_windows_enabled, enabled=True)
init python:
    class TearPiece:
...
screen tear:
    zorder 150
    add Tear(number, offtimeMult, ontimeMult, offsetMin, offsetMax, srf):
        size (1280,720)
    on "show":
        action Function(hide_windows_enabled, enabled=False)
    on "hide":
